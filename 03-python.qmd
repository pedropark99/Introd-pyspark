...
But if the object `x` is an object of a different class, like `int`, then we can no longer use the `location()` method, because this method does not belong to `int` class. For example, if your object is from class `A`, and, you try to use a method from class `B`, you will get an `AttributeError`, like exposed below:
