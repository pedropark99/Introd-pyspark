...
To create our first Spark table we use the `range()` method from the `spark` object. The `range()` method works similarly as the standard python function called `range()`. It basically creates a sequence of numbers, from 0 to $n - 1$. However, this `range()` method stores this sequence of numbers as rows in a Spark table (or a Spark DataFrame):
...
You probably have saved your `spark-example.py` file in a different folder. This means that you need to open the terminal from a different folder. But the process to open it is still the same that is described in @sec-open-terminal.

After I opened a terminal rooted inside the `SparkExample` folder. I just use the `python3` command to access the python interpreter, and, give the name of the python script that I want to execute. In this case, the `spark-example.py` file:
...
Congratulations! You have just run your first Spark application!
