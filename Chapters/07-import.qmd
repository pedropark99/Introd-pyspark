...
## Reading from other static file formats
...
If you want to read a CSV file for example, you use the `spark.read.csv()` engine. In contrast, if you want to read a JSON file, than, you use the `spark.read.json()` engine instead. But no matter what read engine you use, you always give the path to your file to any of these "read engines".
...
Or you can just merge all calls together like this:
...
This is usually the standard adopted by countries that uses commas to define decimal houses in real numbers. In other words, in some countries, the number `3.45` is usually written as `3,45`.
