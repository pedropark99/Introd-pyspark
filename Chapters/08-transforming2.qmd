...
Apart from the six types that we described on the previous section, Spark also offers a seventh type of JOIN called *cross join*. Since this is a special type of JOIN, you can either set the `on` argument at `join()` method to `"cross"`, or, you can also use the `crossJoin()` DataFrame method, to perform this type of JOIN.
