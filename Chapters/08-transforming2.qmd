...
In Spark, pivot operations are performed by the `pivot()` and `stack()` DataFrame methods, and, they are available in both directions. That is, you can transform either rows into columns (corresponds to `pivot()`), or, columns into rows (corresponds to `stack()`). Let's begin with the `stack()` method, and after that, we explain the `pivot()` method.

### Transforming columns into rows

The `stack()` DataFrame method allows you to transform columns into rows. In other words, you can make your DataFrame "longer" with this kind of operation, because you remove columns ("width") from the table, and adds new rows ("heigth"). This gives a aspect of longer to your table, because after this operation, you table usually have more rows than columns.
