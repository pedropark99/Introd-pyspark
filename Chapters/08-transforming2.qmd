...
In essence, a *cross join* between DataFrames A and B returns the cartesian product between these two DataFrames as ouput. It is similar to R functions [`base::expand.grid()`](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/expand.grid) or [`dplyr::expand()`](https://tidyr.tidyverse.org/reference/expand.html), and also, the Python equivalent [`itertools.product()`](https://docs.python.org/2/library/itertools.html#itertools.product).

This a type of JOIN that you should avoid to use, specially if one (or both) of the DataFrames involved is a big DataFrame with thousands/millions of rows. Because a *cross join* will always produce a cartesian product between the two DataFrames involved. This means that, if DataFrame A contains $x$ rows, and DataFrame B contains $y$ rows, the end result of the *cross join* is a new DataFrame that contains $x \times y$ rows.
...
![A visual representation of a pivot process](./../Figures/pivot1.png){#fig-pivot1}
...
Now, the `stack()` function have two main arguments, which are the number of columns to transform into rows, and a sequence of key-value pairs that describes which columns will be transformed into rows, and the label values that corresponds to each column being transformed. As a first example, the source code below replicates the transformation exposed at @fig-pivot1:
...
Differently from other Spark SQL functions, the `stack()` function should not be used inside a `withColumn()`, and the reason for this is very simple: `stack()` always returns two new columns as output (one column for the labels - or the keys, and another for the values), but the `withColumn()` method can only creates a single column. If you try to use `stack()` inside `withColumn()`, you will get an `AnalysisException` error.
