...
In Apache Spark, you perform this kind of operations by using the `collect_list()` and `explode()` functions, which are both from the `pyspark.sql.functions` module. The `collect_list()` function is used for retracting (or nesting) your DataFrame, while the `explode()` function is used for extending (or unnesting).

As a quick comparison, `explode()` is very similar to the R function [`tidyr::unnest_longer()`](https://tidyr.tidyverse.org/reference/unnest_longer.html) from the `tidyverse` framework, and also, similar to the Python method [`explode()`](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.explode.html) in the `pandas` framework. While `collect_list()` does a similar job to the R function [`tidyr::nest()`](https://tidyr.tidyverse.org/reference/nest.html).
