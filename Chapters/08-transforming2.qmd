...
We can remove these duplicated values by using the `distinct()` method. Because on all 3 cases on duplicated values that exist on this DataFrame, the combination of all columns are equal. In the example of transaction ID "T004", all duplicated rows of this ID contains the values `("T004", "Mango", 2, 2.0)`, precisely in this order. Because of that, the `distinct()` method is enough to remove these duplicated values from the table.
...
Now, if we use the `drop_duplicates()` method with only the column `transaction_id` as the key to determine which rows are duplicated, we get a small difference on the result.
...
In the example below, the duplicated values of IDs "T001" and "T004" were removed as we expected. But we also removed the second value for ID "T006". Because we did not listed the `quantity` column on the input list, and, as a result, `drop_duplicates()` was not concerned with the differences on the `quantity` column.

## Replacing `null` values

At @sec-filter-null-values I showed how you can use `filter()` and `where()` DataFrame methods to remove all rows that contained a null value on some column. However, removing an entire row only because there is a null value on one of the columns might not be the best approach. Maybe you prefer to replace this null value for a more useful or representative value, such as `0`, or an empty string (`''`), a `False` value, etc.

To do that in pyspark, we can use the `na.fill()` and `fillna()` DataFrame methods. They both mean the same thing, and work the exact same way. The most popular way of using this methods, is to provide a python dict as input. Inside this dict you have key-value pairs, where the key represents the column name, and the value represents the static value that will replace all null values that are found on the column specified by the key.
