...
At @sec-filter-null-values I showed how you can use `filter()` and `where()` DataFrame methods to remove all rows that contained a null value on some column. However, removing an entire row only because there is a null value on one of the columns might not be the best approach.

Maybe you prefer to replace this null value for a more useful or representative value, such as `0`, or an empty string (`''`), a `False` value, etc. To do that in `pyspark`, we can use the `na.fill()` and `fillna()` DataFrame methods.
