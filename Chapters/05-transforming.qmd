...
### Filtering dates and datetimes in your DataFrame
...
### Using a particular pattern or a regex {#sec-filter-regex-pattern}

In Spark, you can also use a particular pattern or a regex to filter the rows of a DataFrame, by using the `Column` methods `like()` and `rlike()`. Since they are column methods, you must use them after a column name or after a `col()` function.
...
Although the style of pattern matching used by `like()` being very powerful, you might need to use more powerful and flexible tools. In those cases, you can use the `rlike()` method, which accepts a regex (*Regular Expression*) as input. In the example below, I am filtering rows where `destinationBankAccount` starts by the characters `54`.
