...
Writing the logical expression inside a string is the most easy and "clean" way to create a filter expression. However, you could write the same exact expression in a more "formal" way, using the `col()` function from `pyspark.sql.functions`.
...
To create these logical expressions, we use the logical operators of python, which are described in the table below:
...
: List of logical operators of python {#tbl-logical-operators}
