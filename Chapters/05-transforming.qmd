...
To filter specific rows of a DataFrame, `pyspark` offers two equivalent methods called `where()` and `filter()`. In other words, they both do the same thing, and work in the same way. To use 

```{python}
path = "../Data/transf_reform.csv"
transf = spark.read.csv(path)\
  option("sep", ";")
