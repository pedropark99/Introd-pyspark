...
To explicit transform to, or, define which is the correct data type of a column, you can use the `cast()` method inside `select()`. This `cast()` method is very similar to the `CAST` keyword in SQL, and belongs to the `Column` class, so, you should always use it after a column name, or, a `col()` function:
...
This `by_1000` column do not exist in `transf` DataFrame. It was calculated and added to the final result by `select()`. The formula `col('value') / 1000` is the equation that defines what this `by_1000` column is, or, how it should be calculated. 
...
The Spark DataFrame class by itself provides a single aggregate method, which is `count()`. With this method, you can find out how many rows your DataFrame have. In the example below, we can see that `transf` have 20010 rows.
...
In the example below, I am using the `sum()` and `mean()` functions from `pyspark.sql.functions` to calculate the total sum and the total mean of the `value` column in the `transf` DataFrame. I am also using the `countDistinct()` function to calculate the number of distinct values in the `user` column.
...
Remember that, by using standard DataFrame methods (like `max()` in the example above) we can calculate only a single aggregate value. But, with `agg()` we can calculate more than one aggregate value at the same time. Since our `transf_per_user` object is a **grouped** DataFrame, `agg()` will calculate these aggregates per group.

As an example, if I apply `agg()` with the exact same expressions exposed on @sec-agg-method with the `transf_per_user` DataFrame, instead of a DataFrame with one single row, I get a new DataFrame with nine rows. In each row, I have the total and mean values for a specific user in the input DataFrame.
