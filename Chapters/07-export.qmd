...
These are very commom patterns in our data world, and in this chapter, we will discuss the tools that Spark offers to meet these needs.
...
And if I take a look inside this `transf_export` folder I will see two files. One is the placeholder file (`_SUCCESS`), and a CSV file containing the exported data (`part-*`).
...
If we wanted to see this file structure, we can use the [`tree` command line utility](https://www.geeksforgeeks.org/tree-command-unixlinux/)[^tree-cmd] to build a diagram of this file structure:
...
- with `pandas` we can easily export this data into Excel files (`.xlsx`)[^excel-spark].
