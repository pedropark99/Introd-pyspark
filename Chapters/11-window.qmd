...
In order to use a window function you **need to combine a over clause with a window object**. If you pair these two components together, then, you become 
Now that we have defined a window object for our DataFrame, we can actually use window functions, by pairing this window object with an `over()` clause.
