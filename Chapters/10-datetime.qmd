...
Dates are normally interpreted in `pyspark` using the `DateType` data type. There are two commom ways to create date objects, which are from strings (like `"3 of June of 2023"`, or maybe, `"2023-02-05"`) or by extracting the date component from datetime values (i.e. values of type `TimestampType`).
...
The ISO-8601 standard specifies that dates are represented in the format "YYYY-MM-DD" (or "Year-Month-Date"), like `2023-09-19`, `1926-05-21`, or `2005-11-01`. This is also the format that dates are usually formatted in United States of America. If the dates you have (which are currently stored inside strings) are formatted like the ISO-8601 standard, then, you can safely and easily convert them into the `DateType` by using the `cast()` or `astype()` column methods.

However, if these dates are formatted in a different way, then, the `cast()` method will very likely produce `null` values as a result, because it cannot parse dates that are outside the ISO-8601 format.
...
user_events = user_events\
    .withColumn('dateOfEvent', to_date(col('dateOfEvent'), pattern))

user_events.show()
```


## Extracting date and datetime components

One very commom operation when dealing with dates and datetime values is extracting components from the values. Like, you want to create a new column that contains the day component from all of your dates. Or maybe, the month... anyway.

Thakfully, `pyspark` made it pretty easy to extract these kinds of components. You just use the corresponding function to each component! As a quick list, the functions below are used to extract components from both dates and datetime values. All of these functions come from the `pyspark.sql.functions` module:

- `year()`: extract the year of a given date as integer.
- `month()`: extract the month of a given date as integer.
- `dayofmonth()`: extract the day of the month of a given date as integer.
- `dayofweek()`: extract the day of the week of a given date as integer. The integer returned will be inside the range 1 (for a Sunday) through to 7 (for a Saturday).
- `dayofyear()`: extract the day of the year of a given date as integer.


On the other hand, the functions below only applies to datetime values, because they extract time components (which are not present on date values). These functions also comes to the `pyspark.sql.functions` module:
...
In essence, you just apply these functions at the column that contains your date and datetime values, and you get the component you want. As an example, lets go back to the `user_events` DataFrame and extract the day, month and year components of each date, into separate columns:
...
## Adding time to date and datetime values with interval expressions
...
Now, you can use interval expressions to add hours, minutes or seconds to a datetime value. However, interval expressions are only available at the Spark SQL level. As a consequence, you have to use the `expr()` function to get access to Spark SQL to use an interval expression. An interval expression follows the pattern below:

```sql
Column + INTERVAL 3 HOURS
```

A interval expression is an expression that begins with the `INTERVAL` keyword, and then, it is followed by a number that specifies an amount, and at last, another keyword that specifies the units of time to be used. So the expression `INTERVAL 3 HOURS` define a 3 hours interval expression. In contrast, the expression `INTERVAL 3 SECONDS` represents 3 seconds.
