...
The ISO-8601 standard states that datetime values should always follow the format "YYYY-MM-DD HH:MM:SS" (or "Year-Month-Day Hour:Minute:Second"). Examples of values that follows this standard are "2014-10-18T16:30:00Z", "2019-05-09 08:20:00" and "2020-01-01 12:30:00 -03".
...
In the example above, Spark is running on an operating system that is using the Brasília time zone (which is 3 hours late from international time zone - UTF-3) as the "default time zone" of the system. As a result, integers will be interpreted as being the number of seconds since the UNIX time epoch **minus 3 hours**, which is `"1969-12-31 21:00:00"`. So, in this context, the integer `60` would be converted into `"1969-12-31 21:01:00"` (instead of the usual `"1970-01-01 00:01:00"` that you would expect).
