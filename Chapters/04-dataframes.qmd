...
Remember that a Spark DataFrame in python is a object of class `pyspark.sql.dataframe.DataFrame` as you can see below. If you try to see what is inside in this kind of object, you will get a small description of the columns present in the DataFrame as a result:
...
We can access the read engines for these different file formats, by using the `read` module from your Spark Session object. For example, I have the following CSV file saved in my computer:
...
This CSV was saved in a file called `people.csv`, inside a folder called `Data`. So, to read this file, I give the path to this CSV file to the `read.csv()` method of my Spark Session, like in the example below:
...
Some of the many different read engines available in spark are listed below:
...
### Defining some import options

The `spark.read.csv()` method, Spark will use the default options to read and import your CSV files. But, you may want to change these default options at some point.

For example, the `people.csv` that we shown above, has no column names.
