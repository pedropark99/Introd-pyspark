...
As an example, in the code below I expose all the available methods from this `DataFrame` class. First, I am creating a Spark DataFrame with `spark.range(5)`, and, storing it in the object `df5`. After that, I use the `dir()` function to show all the methods that I can use through this `df5` object:
...
Other examples are the methods used to import data to `pyspark`. These methods are available in the `spark.read` module, like `spark.read.csv()` and `spark.read.json()`. These methods will be described in more depth in the @sec-import-export.
...
You can notice this laziness in the above outputs. Because when we call for an object that stores a Spark DataFrame (like `df` and `students`), Spark will only calculate and print a summary of the structure of your Spark DataFrame, and not the DataFrame itself.
...
Each column of your Spark DataFrame is associated with a specific data type. Spark supports a number of different data types. You can see the full list at the official documentation page[^04-dataframes-1]. For now, we will focus on the most used data types, which are listed below:

[^04-dataframes-1]: spark-data-types1
...
This unexpected result happens, because the `id` column is actually from the "big integer" type, or, the `LongType` type (which are 8-byte signed integer). You can see below, that now the test results in true:
...
You will see some of the methods from this `Column` class across the next chapters, like `desc()`, `alias()` and `cast()`. For now, just be aware that these class of objects exists, and, that they will be very useful.
