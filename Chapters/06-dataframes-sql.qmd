...
As I noted at @sec-columns-related-expressions, columns of a Spark DataFrame are closely related to expressions. As a result, you usually use and execute expressions in Spark when you want to transform (or mutate) columns of a Spark DataFrame.
...
There are many column transformations that are particularly verbose and hard to write in `pyspark`. But you can use a SQL expression in your favor, to translate this transformation into a more short and concise form. Virtually every expression you write in `pyspark` can be translated into a SQL expression.
