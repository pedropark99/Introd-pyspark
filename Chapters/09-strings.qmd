...
The second argument is the amount of characters in the substring, or, in other words, it's length. For example, if you set this argument to 6, it means that the function will extract the substring that is formed by walking $6 - 1 = 5$ characters ahead from the start position you specified at the first argument. We can also interpret this as: the function will walk ahead on the string, from the start position, until it gets a substring that is 6 characters long.
...
There is also a column method that provides an useful way of testing if the values of a column matchs a regular expression or not, which is the `rlike()` column method. So I can use the `rlike()` method in conjunction with the `filter()` or `where()` DataFrame methods, to find all values that fit (or match) a particular regular expression.
...
- `\w` : Matches any word character. Equivalente to the "\b([a-zA-Z_0-9]+)\b" regular expression;
...
### Replacing occurrences of a particular regular expression
...
The `regexp_extract()` function in PySpark is a string manipulation function that extracts substrings from a given string that match a specified regular expression pattern.
...
# Create a PySpark DataFrame with a column called "text"
df = spark.createDataFrame([('John Smith 123 Main St.'],), ['text'])

# Extract the name from the "text" column
df = df.withColumn('name', regexp_extract('text', '(\w+)\s\w+\s\d+\s\w+\.\s*', 1))

# Show the result
df.show()
```

In the above example, the regexp_extract() function searches for a regular expression pattern that matches a person's name followed by their address. The capturing group (\w+) extracts the name from the input string. The resulting DataFrame would look like this:


This function can be useful for extracting specific information from structured text data in PySpark. Note that if the capturing group index is invalid or if there is no match for the regular expression pattern in the input string, the function returns a null value.

### How to use groupings on `pyspark`

One of the many awesome functionalities of regular expressions, is the capability of enclosing parts of a regular expression inside groups. This process of capturing parts of a regular expression inside a group, is usually called of "grouping".

Ok, but, what is this group thing? You create a group inside a regular expression by enclosing a particular section of your regular expression inside a pair of parentheses. A group is created with the regular expression that is written inside this pair of of parentheses.

A group inside a regular expression is used to capture a specific part of the matched string. This means that the actual part of the input string that is matched by the regular expression that is inside this pair of parentheses, is captured (or cached, or saved) by the group, and, can be reused later.

So, remember, to use groupings in a regular expression, you need to enclose the part of the pattern that you want to capture in parentheses `()`. Each set of parentheses creates a new capturing group. This means that you can create multiple groups inside a single regular expression, and, than, reuse the substrings captured by all of these multiple groups. Awesome, right?

In addition to capturing groups, regular expressions can also have non-capturing groups. A non-capturing group is defined using (?:...). It is used to group a pattern without capturing it. Non-capturing groups are often used to apply a quantifier to a group without capturing the group itself.

Here's an example of how to use a group inside a regular expression in Python:
...
logs\
    .withColumn(
        'without_type',
        regexp_replace('message', type_regex, '$1')
    )\
    .select('message', 'without_type')\
    .show(truncate = 30)
```
