...
The `rlike()` column method is useful for checking if a string value in a column matches a specific regular expression. We briefly introduced this methd at @sec-filter-regex-pattern. This method has only one input, which is the regular expression you want to apply over the column values.
