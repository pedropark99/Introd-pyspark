...
Ok, now that we understood what capturing groups is, how can we use them in `pypspark`? First, remember, capturing groups will be available to you, only if enclose a part of your regular expression in a pair of parentheses. So the first part is to make sure that the capturing groups are present in your regular expressions.
...
The `rlike()` column method is used to check if the a string value in a column matches a specified regular expression. We briefly introduced this methd at @sec-filter-regex-pattern. This method has only one input, which is the regular expression you want to apply over the column values.
