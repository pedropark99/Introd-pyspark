...
### Replacing occurrences of a particular regular expression with `regexp_replace()`
...
One of the many awesome functionalities of regular expressions, is the capability of enclosing parts of a regular expression inside groups, and actually storing (or caching) the substring matched by this grouped. This process of grouping parts of a regular expression inside a group, is usually called of "grouping and capturing".
...
Each new group (that is, each pair of parentheses) that you create in your regular expression have a different index. That means that the first group is identified by the index 1, the second group, by the index 2, the third group, by the index 3, and goes on.
...
#### How can we use groupings in `pyspark` ?
